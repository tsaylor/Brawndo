...
class ApiTest(unittest.TestCase):
    ...
    def testGetPublicTimelineNew(self):
        '''Test the twitter.Api GetPublicTimeline method'''
        self._AddHandler('http://twitter.com/statuses/public_timeline.json',
                         curry(self._OpenTestData, 'getpublictimeline.json'))
        statuses = self._api.GetPublicTimeline()
        # This is rather arbitrary, but spot checking is better than nothing
        self.assertEqual(20, len(statuses))
        self.assertEqual(3129942355L, statuses[0].id)
  
    def testGetPublicTimelineGzip(self):
        '''Test the twitter.Api GetPublicTimeline method using GZIP Compression'''
        self._AddHandler('http://twitter.com/statuses/public_timeline.json?since_id=12345',
                         curry(self._OpenTestData, 'public_timeline.json.gz'))
        statuses = self._api.GetPublicTimeline(since_id=12345)
        # This is rather arbitrary, but spot checking is better than nothing
        self.assertEqual(20, len(statuses))
        self.assertEqual(89497702, statuses[0].id)
    ...
    def testGetMentions(self):
        '''Test the twitter.Api GetMentions method'''
        self._AddHandler('http://twitter.com/statuses/mentions.json?page=1',
                         curry(self._OpenTestData, 'mentions.json'))
        statuses = self._api.GetMentions(page=1)
    ...
    def testGetFollowersGzip(self):
        '''Test the twitter.Api GetFollowers method using GZIP Compression'''
        self._AddHandler('http://twitter.com/statuses/followers.json?page=1',
                         curry(self._OpenTestData, 'followers.json.gz'))
        users = self._api.GetFollowers(page=1)
        # This is rather arbitrary, but spot checking is better than nothing
        alexkingorg = [u.status for u in users if u.screen_name == 'alexkingorg']
        self.assertEqual(89554432, alexkingorg[0].id)
    ...
    def testGetCreatedAtInSeconds(self):
        '''Test the twitter.Api GetPublicTimeline method'''
        self._AddHandler('http://twitter.com/statuses/public_timeline.json',
                         curry(self._OpenTestData, 'getpublictimeline.json'))
        statuses = self._api.GetPublicTimeline()
    ...
    def _OpenTestData(self, filename, headers=[]):
        return MockResponce(self._GetTestDataPath(filename), headers)
    

class MockHeaders(dict):
    def get(self, a, b):
        if self._gzip == True: return "gzip"
        else: return b
...
class MockResponce(object):
    def __init__(self, file,headers):
        ...
        if ".gz" in file: self.headers._gzip = True
        else: self.headers._gzip = False
    def __str__(self):
        return self.data
    def read(self):
        return self.f.read()
