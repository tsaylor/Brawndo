...
CONSUMER_KEY = getattr(settings, 'CONSUMER_KEY', 'Bsau1s8cE9Ms7VOtG5pKg')
CONSUMER_SECRET = getattr(settings, 'CONSUMER_SECRET', 'MkiBScQAsB6NLw2eeEz7hnNDAYgt0nt1ATz52OFE1U')

# We use this URL to check if Twitters oAuth worked
TWITTER_CHECK_AUTH = 'https://twitter.com/account/verify_credentials.json'
TWITTER_FRIENDS = 'https://twitter.com/statuses/friends.json'
TWITTER_FAVORITES = 'https://twitter.com/favorites/%s.json' # the %s is username
...
def get_favorites(consumer, connection, access_token, page=0, username=None):
    """Get <username>'s favorited tweets"""
    if username == None:
	return []
    oauth_request = request_oauth_resource(consumer, TWITTER_FAVORITES % (username), access_token, {'page': page})
    json = fetch_response(oauth_request, connection)
    return json
